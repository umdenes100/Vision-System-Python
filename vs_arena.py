...
def processMarkers(frame, marker_list, H,inverse_matrix, dr_op):
    markers = {}
    markers[f'{0}'] = processed_Marker(0, -1.0, -1.0, -1.0)
    markers[f'{1}'] = processed_Marker(1, -1.0, -1.0, -1.0)
    markers[f'{2}'] = processed_Marker(2, -1.0, -1.0, -1.0)
    markers[f'{3}'] = processed_Marker(3, -1.0, -1.0, -1.0)
    try:
        for x in marker_list:
            if x.id > 3:
